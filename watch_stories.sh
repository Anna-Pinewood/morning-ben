#!/usr/bin/env bash
# Morning Ben — вотчер очереди, единственный механизм генерации: каждые
# 10 секунд считает непоказанные истории (stories/*.json минус
# shown_history) и, если их меньше порога, догенерирует пачку. Так очередь
# постоянно держится на уровне порога, а генерится ровно столько, сколько
# реально читается. Кулдаун защищает от цикла перезапусков, если генерация
# падает.
#
# Переменные окружения:
#   MIN_UNSEEN — порог (default 15): триггер, когда непоказанных меньше
#   TOPUP_N    — сколько историй догенерировать за раз (default 5)
set -euo pipefail
cd "$(dirname "$0")"

MIN_UNSEEN="${MIN_UNSEEN:-15}"
TOPUP_N="${TOPUP_N:-5}"
COOLDOWN=600
last_run=0
