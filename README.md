...
Она создаёт tmux-сессию **`morning-ben`** с тремя окнами; каждое обёрнуто
в перезапускающий цикл — упавший процесс сам поднимется через 5 секунд:

| Окно | Что делает | Лог |
|------|-----------|-----|
| 0 `bot` | телеграм-бот, long polling | `logs/bot.log` |
| 1 `watcher` | `watch_stories.sh`: раз в 10 с считает непоказанные истории; меньше `MIN_UNSEEN` (2) — запускает `generate.sh` на `TOPUP_N` (5); кулдаун 10 мин | `logs/watcher.log` |
| 2 `nightly` | `nightly_generate.sh 20`: вечный цикл, каждый день в **21:03 по Москве** генерирует 20 историй (таймзона зашита в скрипт, серверная не важна) | `logs/nightly.log` + `logs/generate.log` |

Шпаргалка:

```bash
tmux attach -t morning-ben                           # посмотреть; Ctrl-b 0/1/2 — окна, Ctrl-b d — выйти
