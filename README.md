...
- `state/feedback.json` — обратная связь: каждое свободное сообщение боту
  ...
  Генератор различает тип пожелания (правила — раздел «Как понимать
  обратную связь» в `generate_stories.md`): тематическое («хочу про
  асаны») увеличивает долю темы примерно в полтора раза (а если таких
  пожеланий несколько — каждая тема просто гарантированно попадает в
  пачку), не вытесняя остальные направления; стилевое («пиши научнее»)
  применяется ко всем историям целиком.
