...
## Авто-пополнение очереди

**`watch_stories.sh`** — вотчер: раз в 10 секунд считает непоказанные
истории; когда их меньше порога (`MIN_UNSEEN`, default 2), запускает
`generate.sh` на `TOPUP_N` историй (default 5). Кулдаун 10 минут защищает
от цикла при падающей генерации. На сервере работает как systemd
user-сервис `morning-ben-watcher.service`, лог — `logs/watcher.log`.
